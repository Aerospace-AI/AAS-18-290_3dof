...
class Lander_model(object):
    ...
    def __init__(self, use_trajectory_list=False, divert = (0.0,  0.0, 0.0), 
    ...
    def get_state_agent3(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'])
        state = np.hstack((error, t_go, self.state['mass']))
        self.state['t_go'] = t_go
        return state 
    ...
    def get_state_agent6(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'])
        state = np.hstack((error, t_go, self.state['position'][2], self.state['mass']))
        self.state['t_go'] = t_go
        return state
    ...
    def get_state_agent10(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'])
        self.state['t_go'] = t_go
        state = np.hstack((error, self.state['position'][2]))
        return state 
    ...
    def show_final_stats(self,type='final'):
        ...
        stats = {}
        keys = ['position', 'velocity', 'fuel', 'glideslope']
        formats = {'position' : '{:8.1f}' , 'velocity' : '{:8.3f}', 'fuel' : '{:6.2f}', 'glideslope' : '{:6.2f}'}
    ...
    def get_engagement_keys(self):
        keys = ['t', 'position', 'velocity', 'thrust', 'mass', 'fuel', 'vc', 'range', 'los', 'dlos', 'zem','reward','fuel_reward','landing_reward','tracking_reward','glideslope_reward', 'landing_margin', 'glideslope', 'sc_margin', 'glideslope_penalty','sc_penalty','sc_reward','range_reward','v_ratio', 't_go']
        return keys
    ...
    def apf_pot_pv2(self,state):
        ...
        if rg[2] < self.apf_atarg:
            rg = 1.0*np.asarray([0,0,state[2]])
            vg = state[3:6] - self.apf_vf2
            mag_vg = np.linalg.norm(vg)
            mag_rg = np.linalg.norm(rg)
        else:
            rg = state[0:3] - 1.0*np.asarray([0,0,self.apf_atarg])
    ...
    def apf_pot_foo(self,state):
        ...
        if rg[2] > self.apf_atarg:
            ...
            pot = np.asarray([0. , 0., -1])
            t_go = -10.

        return pot, t_go

    def apf_pot_cs(self,state):
        rg = state[0:3]
        vg = state[3:6]
        mag_rg = np.linalg.norm(rg)
        mag_vg = np.linalg.norm(vg)
        ...
        if xy < 10:
            rg1 = state[0:3] - 1.0*np.asarray([0,0,self.apf_atarg])
            rg2 = 1.0*np.asarray([0,0,state[2]]                 
            traj = np.vstack((rg1,rg2))
            xys = np.vstack((10.0, 0.0))
            for i in range(3):
                f = interp1d(self.time_samples, self.trajectory_samples[:,i], kind='cubic')
    ...
    def apf_pot_g(self,state):
        ...
        if rg[2] > self.apf_atarg:
            ...
            tau = self.apf_tau1
            t_go = mag_rg1 / mag_vg1
            mag_pot =  self.apf_v0 * (1. - np.exp(-t_go**2 / tau**2))
        else:
            rg1 = 1.0*np.asarray([0,0,state[2]])
            vg1 = state[3:6] - self.apf_vf2
            mag_vg1 = np.linalg.norm(vg1)
    ...
    def apf_pot100(self,state):
        rg = state[0:3]
        vg = state[3:6]
        mag_rg = np.linalg.norm(rg)
        mag_vg = np.linalg.norm(vg)

        if rg[2] < self.apf_atarg and not self.apf_state:
            self.apf_cnt = self.apf_tau2  # sticky
        elif self.apf_state and self.apf_cnt > 0:
            self.apf_cnt -= 1
        ...
        if self.apf_state:
            t_go = self.apf_cnt
            mag_pot =  self.apf_v0 * (1. - np.exp(-t_go / tau)) 
            #print('DEBUG: ',rg[2], self.apf_cnt, self.apf_state, vg)       
        else:
            mag_pot =  self.apf_v0 * (1. - np.exp(-t_go / tau))
        dir_pot = -rg1 / mag_rg1
