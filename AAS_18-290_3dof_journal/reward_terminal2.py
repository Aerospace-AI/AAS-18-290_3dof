...
class Reward(object):
    ...
    def __init__(self, reward_scale=1.0, ts_coeff=-0.0,  fuel_coeff=-0.05, term_fuel_coeff=-0.0,
                 ...
                 landing_range_coeff=-0.0, landing_quad_coeff=0.0, use_q1=True,
        ...
        self.landing_range_coeff =  landing_range_coeff
        self.landing_quad_coeff =   landing_quad_coeff
        self.use_q1 =               use_q1
    ...
    def get(self, lander,  action, done, steps, shape_constraint, glideslope_constraint):
        ...
        if done:
            ...
            if np.linalg.norm(pos) < self.landing_rlimit and np.linalg.norm(vel) < self.landing_vlimit and pos[2] < 0.0: 
                r_landing = np.clip(glideslope_constraint.get(), 0, 10)
                r_landing += (self.landing_quad_coeff / 2) * self.quad_reward2(pos,self.landing_rlimit) + (self.landing_quad_coeff / 2) * self.quad_reward2(vel,self.landing_vlimit)
        ...
        reward = (r_range + sc_penalty + gs_penalty + r_gs + r_sc +  r_landing + r_tracking +  r_fuel + self.ts_coeff) * self.reward_scale
        ...
        lander.trajectory['range_reward'].append(r_range)
